...
const Header = ({size = AVATAR_DEFAULT_SIZE}: Props) => {
    ...
    return <div className="grid grid-cols-2 shadow-lg hover:drop-shadow-xl bg-gray-100 px-5 sticky top-0 z-50 h-14">
        ...
        {!loading && data?.me.displayName &&
            <div className="flex flex-row justify-end p-2.5">
                <Avatar name={data.me.displayName} round={true} size={size.toString()}/>
                <button type="button"
                        className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-1.5 mx-2 ark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700"
                        onClick={logout}>
